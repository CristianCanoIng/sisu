import{requireProfile}from'../session.js';import{supabase}from'../supabase.js';import{renderNavbar}from'../navbar.js';import{escapeHtml,formatDateTime,badgeClass,showAlert,openModal,closeModal,wireModal,$}from'../utils.js';
...
function render(){
 ...
 $('#topActions').innerHTML='<button id="newLoan" class="btn btn-success"><i class="fas fa-handshake"></i> Registrar préstamo</button><button id="newItem" class="btn btn-primary"><i class="fas fa-plus"></i> Nuevo implemento</button>';
 ...
 $('#newItem').onclick=openNewItem;$('#newLoan').onclick=()=>{if(!items.some(i=>i.estado==='Activo'&&Number(i.cantidad_disponible)>0))return showAlert('No hay implementos disponibles para prestar.','error');openModal('loanModal')};
