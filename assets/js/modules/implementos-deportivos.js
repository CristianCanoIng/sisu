...
async function load(){
 const [ir,lr,sr]=await Promise.all([
  ...
  loadOperationalStudents()
 ]);
 if(ir.error)throw ir.error;if(lr.error)throw lr.error;
 items=ir.data||[];loans=lr.data||[];students=sr||[];render();
}
function itemModal(){
...
function loanModal(){
 const studentOpts=studentOptions(students.filter(s=>s.estado==='Activo'),'id_usuario');
...
function render(){
 ...
 $('#itemForm').onsubmit=saveItem;$('#loanForm').onsubmit=saveLoan;$('#loanStudentSearch').oninput=e=>filterStudentSelect($('#estudiante'),students.filter(s=>s.estado==='Activo'),e.target.value,'id_usuario');$('#loCreate').onclick=quickCreateStudent;
...
function exportLoans(){downloadCsv('prestamos_implementos_'+new Date().toISOString().slice(0,10)+'.csv',loans,[{label:'ID',value:'id_prestamo'},{label:'ID estudiante',value:r=>studentName(r.id_estudiante).codigo_estudiantil||''},{label:'Estudiante',value:r=>studentName(r.id_estudiante).nombre||''},{label:'Documento',value:r=>studentName(r.id_estudiante).documento||''},{label:'Implemento',value:r=>itemName(r.id_implemento).nombre||''},{label:'Codigo',value:r=>itemName(r.id_implemento).codigo||''},{label:'Cantidad',value:'cantidad'},{label:'Fecha_prestamo',value:'fecha_prestamo'},{label:'Fecha_devolucion',value:'fecha_devolucion'},{label:'Estado',value:'estado'},{label:'Observaciones_salida',value:'observaciones_salida'},{label:'Observaciones_entrega',value:'observaciones_entrega'}])}
...
async function quickCreateStudent(){
 try{
  const result=await createQuickStudent('lo');
  await load();
  openModal('loanModal');
  $('#estudiante').value=String(result.id_usuario);
