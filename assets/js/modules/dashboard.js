...
async function renderGeneral(){
 ...
 const [{data:incs},{data:cons},{data:acts}]=await Promise.all([
  supabase.from('incapacidades').select('id_incapacidad,fecha_inicio,estado,pacientes(codigo_estudiantil,programa_academico,usuarios(nombre))').order('fecha_radicacion',{ascending:false}).limit(5),
  supabase.from('consultas').select('id_consulta,fecha,motivo,estado,pacientes(codigo_estudiantil,programa_academico,usuarios(nombre))').order('fecha',{ascending:false}).limit(5),
